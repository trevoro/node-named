...
// * add CRUD for records during runtime (or just let a pluggable back-end do
//   this)
// * add cache for serialized records (how much would this actually save?)
  ...
  switch(opts.type) {
    case protocol.QTYPE_A:
      ...
      if (!records.hosts[host]) 
        return { error: { message: 'No entry: ' + opts.name, code: protocol.DNS_ENONAME } };
