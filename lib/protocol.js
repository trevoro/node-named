...
var serializers = {
  ...
  '_nsName': {
		encoder: function(v) {
			console.log("nsname encoder: encoding: %s", v);
      if (typeof(v) !== 'object') 
      	throw new TypeError('v (object) is required')
		...
		encoder: function(v, t) {
		  ...
		  switch(t) {
				...
				case QTYPE_MX:
				  ...
				  break;
				case QTYPE_TXT:
				  r = serializers['_nsText'].encoder(v);
				  break;
				case QTYPE_AAAA:
				  r = serializers['_nsIP6'].encoder(v);
				  break;
			}
			console.log(r);
			l = r.length;
			console.log("Size of resulting buffer is %s bytes", l);
			b = new Buffer(l + 2);
      b.writeUInt16BE(l, 0);
      r.copy(b, 2);
      console.log(b);
...
var encode = function(obj, format) {
  ...
  for (f in fmt) { 
  	...
  	type = fmt[f].type;
		...
		console.log("FIELD IS: %s", f);
		console.log("VALUE IS: %s", obj[f]);
