...
Server.prototype.listen = function listen(port, address, callback) {
	...
	var sock_type = 'udp6';
	if (ipaddr.parse(address).octets.length == 4) {
		// a udp6 socket will create both IPv4 and IPv6 sockets
		// but if the address is IPv4 we need to bind only to a udp4 socket
		sock_type = 'udp4';
	}

        this._socket = dgram.createSocket(sock_type);
