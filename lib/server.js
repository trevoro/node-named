...
Server.prototype.send = function (res) {
  ...
  this._log.debug("Sending DNS response message to %s:%s", 
      res._source.address, res._source.port);
  
  this._socket.send(res._raw.buf, 0, res._raw.len, 
      res._source.port, res._source.address);
};
      ...
      port: rinfo.port
		...
		try {
			...
			try {
				...
				self.emit('clientError', new DNSError.CannotProcessError('unable to respond to query'));
			}
		}
		catch (e) {
		  self.emit('clientError', new DNSError.ProtocolError('invalid DNS datagram'));
	  }
