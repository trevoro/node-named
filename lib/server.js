...
Server.prototype.send = function (res) {
  ...
  try {
    ...
    self.emit('uncaughtException', new DnsError.ExceptionError('unable to encode response'));
    return false;
  }
...
Server.prototype.listen = function (port, address, onListen) {
  ...
  if (onListen)
    this._socket.addListener('listening', onListen);
...
Server.prototype.close = function(callback) {
  //this._socket.addListener('close', callback);
  this._socket.close();
  if (callback) 
    return callback();
}
